...
#####
lm.lintemp <- lm(kwh ~ temperature + hrstr + working_day + price + hrstr:working_day,
                     data = readings.aggregate,
                     weights = agg_count)
...
#####
lm.ns <- lm(log(kwh) ~ ns(temperature, knots = c(2, 24, 30)) + hrstr + working_day + price + hrstr:working_day + bs(hoy_frac, df = hoydf), 
            data = readings.aggregate,
            weights = readings.aggregate$agg_count)
...
ns.visreg <- visreg(fit = lm.ns, xvar = "temperature", type = "conditional",
                       ...
                       ylab = "Conditional Electricity Demand (kWh)",
                       xlab = "Outdoor Temperature (Celsius)")
ns.temp <- bs(readings.aggregate$temperature, knots = c(2, 24, 30))
