...
# plot(seg, 
#      res = TRUE, 
#      res.col = rgb(0, 0, 100, 10, maxColorValue=100),
#      pch = 16,
#      main = "Piecewise Linear Fit with Temperature Breakpoints",
#      xlab = "Summer Temperatures (Celsius)",
#      ylab = "Transformed Observations [log(kWh)]",
#      rug = FALSE,
#      lwd = 0)
# 
...
poly.vars <- poly(readings.aggregate$temp_lag2, degree = 4)
colnames(poly.vars) <- c("temp_poly1", "temp_poly2", "temp_poly3", "temp_poly4")
...
nrow(bad.resids)
library(xlsx)
write.xlsx(bad.resids, "bad_resids.xlsx")