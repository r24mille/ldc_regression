...
#####
# Add seasonality term and maybe use it.
# TODO(r24mille): See if seasonality term is required after fitting non-linear methods to conditional temperature effects.
#####
hoydf <- 7
scatter.black = rgb(0, 0, 0, 50, maxColorValue = 100)
lm.seasonality <- lm(kwh ~ temperature + bs(hoy_frac, df = hoydf) + 
                   hrstr + working_day + price + hrstr:working_day,
                 data = readings.aggregate,
                 weights = readings.aggregate$agg_count)
seasonality.visreg <- visreg(fit = lm.seasonality, xvar = "hoy_frac", type = "conditional",
                             alpha = 1, # no confidence interval
                             line.par = list(col = "red", lwd = 3),
                             points.par = list(col = scatter.black),
                             main = "Annual Seasonality Conditioned on Other Explanatory Variables (Cubic Splines)", 
                             ylab = "Conditional Electricity Demand (kWh)",
                             xlab = "Fraction of Year Completed (by hour)")
bs.hoy <- bs(readings.aggregate$hoy_frac, df = hoydf)
abline(v = attr(bs.hoy, "knots"), col = "blue", lty = 2, lwd = 1)
legend("topleft", lty = c(1, 2), lwd = c(3, 1), col = c("red", "blue"), legend = c("Cubic Spline Fit", "Knot Position"))
dev.print(file="../../Figures/SeasonalityConditionalPlot.png", device=png, height = 600, width = 800)
...
#####
lm.lintemp <- lm(kwh ~ temperature + hrstr + working_day + price + hrstr:working_day + bs(hoy_frac, df = hoydf),
                     data = readings.aggregate)
...
#####
par(mfrow = c(2, 2))
poly2 <- lm(kwh ~ poly(temperature, degree = 2) + hrstr + working_day + price + hrstr:working_day + bs(hoy_frac, df = hoydf), 
            data = readings.aggregate,
            weights = readings.aggregate$agg_count)
...
poly3 <- lm(kwh ~ poly(temperature, degree = 3) + hrstr + working_day + price + hrstr:working_day + bs(hoy_frac, df = hoydf), 
            data = readings.aggregate,
            weights = readings.aggregate$agg_count)
...
poly4 <- lm(kwh ~ poly(temperature, degree = 4) + hrstr + working_day + price + hrstr:working_day + bs(hoy_frac, df = hoydf), 
            data = readings.aggregate,
            weights = readings.aggregate$agg_count)
...
poly5 <- lm(kwh ~ poly(temperature, degree = 5) + hrstr + working_day + price + hrstr:working_day + bs(hoy_frac, df = hoydf), 
            data = readings.aggregate,
            weights = readings.aggregate$agg_count)
...
#####
lm.ns <- lm(kwh ~ ns(temperature, knots = c(9, 26, 31)) + hrstr + working_day + price + hrstr:working_day + bs(hoy_frac, df = hoydf), 
            data = readings.aggregate,
            weights = readings.aggregate$agg_count)
...
ns.visreg <- visreg(fit = lm.ns, xvar = "temperature", type = "conditional",
                       ...
                       ylab = "Conditional Electricity Demand (kWh)",
                       xlab = "Outdoor Temperature (Celsius)")
ns.temp <- bs(readings.aggregate$temperature, knots = c(9, 26, 31))
...
#####
# Compare the temperature fit with and without temperature
#####
lm.ns.withseason <- lm(kwh ~ ns(temperature, knots = c(9, 26, 31)) + hrstr + working_day + price + hrstr:working_day + bs(hoy_frac, df = hoydf), 
                       data = readings.aggregate,
                       weights = readings.aggregate$agg_count)
withseason.contrast <- visreg(fit = lm.ns.withseason, xvar = "temperature", type = "contrast")

lm.ns.noseason <- lm(kwh ~ ns(temperature, knots = c(2, 24, 33)) + hrstr + working_day + price + hrstr:working_day, 
                       data = readings.aggregate,
                       weights = readings.aggregate$agg_count)
noseason.contrast <- visreg(fit = lm.ns.noseason, xvar = "temperature", type = "contrast")


plot(x = withseason.contrast$fit$temperature, 
     y = withseason.contrast$fit$visregFit,
     type = "l",
     lwd = 3,
     col = "blue",
     ylim = c(-0.25, 2.5),
     main = "Conditional Contrasts for Temperature Splines With/Without Seasonal Term",
     ylab = "Change in kWh",
     xlab = "Outdoor Temperature (Celsius)")
lines(x = noseason.contrast$fit$temperature, 
     y = noseason.contrast$fit$visregFit,
     lwd = 3, 
     col = "green")
legend("topleft", lty = c(1, 1), lwd = c(3, 3), col = c("blue", "green"), 
       legend = c("Temperature Spline Fit (Seasonal Term in Model)", "Temperature Spline Fit (No Seasonal  Term in Model)"))
dev.print(file="../../Figures/ResidualComparison/TemperatureSplinesSeasonalTermComparison.png", device=png, height = 600, width = 800)
...
cb <- crossbasis(x = readings.aggregate$temperature, 
                 argvar = list(fun = "ns", knots = c(9, 26, 31)),
                 lag = 16, 
                 arglag = list(fun = "ns", df = 3))

# Prediction using the crossbasix matrix
lm.cb <- lm(kwh ~ cb + hrstr + working_day + price + hrstr:working_day + bs(hoy_frac, df = hoydf),
            data = readings.aggregate)
...
BIC(lm.cb)
dwtest(lm.cb)
coeftest(lm.cb, vcov = NeweyWest(lm.cb, lag = 3))
lm.cb.mape <- mape(readings.aggregate[c(17:nrow(readings.aggregate)),]$kwh, predict(lm.cb))
plot(density(lm.cb$residuals, bw = 0.014),
     main = "Distribution of Residuals \n(no kWh transform, with seasonality term)",
     ylim = c(0,4.75), 
     xlim = c(-1.6,0.5),
     lwd = 3)
#dev.print(file = "../../Figures/ResidualComparison/Season/ResidualDistribution.png", device = png, height = 600, width = 800)
...
plot(x = lm.cb$fitted, y = lm.cb$residuals,
     ...
     ylab = "Residual",
     ylim = c(-1.8, 1.1))
#dev.print(file = "../../Figures/ResidualComparison/Season/ResidualDiagnostics.png", device = png, height = 600, width = 800)
...
plot(lm.cb$residuals,
     ...
     ylim = c(-1.8, 1.1))
#dev.print(file = "../../Figures/ResidualComparison/Season/ResidualSerialCorrelation.png", device = png, height = 600, width = 800)
...
lm.intuitive.me <- lm(kwh ~ poly(temperature, degree = 3) + month + working_day + hrstr + price + severe_weather,
                      data = readings.aggregate)
