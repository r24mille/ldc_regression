...
lm.seasonality <- lm(log(kwh) ~ temperature + bs(hoy_frac, df = hoydf) + 
...
#####
lm.lintemp <- lm(log(kwh) ~ temperature + hrstr + working_day + price + hrstr:working_day + bs(hoy_frac, df = hoydf),
                     data = readings.aggregate)
...
#####
par(mfrow = c(2, 2))
poly2 <- lm(log(kwh) ~ poly(temperature, degree = 2) + hrstr + working_day + price + hrstr:working_day + bs(hoy_frac, df = hoydf), 
            data = readings.aggregate,
            weights = readings.aggregate$agg_count)
...
poly3 <- lm(log(kwh) ~ poly(temperature, degree = 3) + hrstr + working_day + price + hrstr:working_day + bs(hoy_frac, df = hoydf), 
            data = readings.aggregate,
            weights = readings.aggregate$agg_count)
...
poly4 <- lm(log(kwh) ~ poly(temperature, degree = 4) + hrstr + working_day + price + hrstr:working_day + bs(hoy_frac, df = hoydf), 
            data = readings.aggregate,
            weights = readings.aggregate$agg_count)
...
poly5 <- lm(log(kwh) ~ poly(temperature, degree = 5) + hrstr + working_day + price + hrstr:working_day + bs(hoy_frac, df = hoydf), 
            data = readings.aggregate,
            weights = readings.aggregate$agg_count)
...
#####
lm.ns <- lm(log(kwh) ~ ns(temperature, knots = c(3, 23, 30)) + hrstr + working_day + price + hrstr:working_day + bs(hoy_frac, df = hoydf), 
            data = readings.aggregate,
            weights = readings.aggregate$agg_count)
...
ns.visreg <- visreg(fit = lm.ns, xvar = "temperature", type = "conditional",
                       ...
                       ylab = "Conditional Electricity Demand, log(kWh)",
                       xlab = "Outdoor Temperature (Celsius)")
ns.temp <- bs(readings.aggregate$temperature, knots = c(3, 23, 30))
...
cb <- crossbasis(x = readings.aggregate$temperature, 
                 argvar = list(fun = "ns", knots = c(3, 23, 30)),
                 lag = 12, 
                 arglag = list(fun = "lin"))

# Prediction using the crossbasix matrix
lm.cb <- lm(log(kwh) ~ cb + hrstr + working_day + price + hrstr:working_day + bs(hoy_frac, df = hoydf),
            data = readings.aggregate)
...
plot(density(residuals.bktr, bw = 0.014),
     main = "Distribution of Residuals \n(backtransformed, with seasonality term)",
     ylim = c(0,4.75),
     xlim = c(-1.6,0.5),
     lwd = 3)
dev.print(file = "../../Figures/ResidualComparison/SeasonBacktransformed/ResidualDistribution.png", device = png, height = 600, width = 800)
...
plot(x = exp(predict(lm.cb)), y = residuals.bktr,
     ...
     ylab = "Residual",
     ylim = c(-1.8, 1.1))
dev.print(file = "../../Figures/ResidualComparison/SeasonBacktransformed/ResidualDiagnostics.png", device = png, height = 600, width = 800)
...
dev.print(file = "../../Figures/ResidualComparison/SeasonBacktransformed/ResidualSerialCorrelation.png", device = png, height = 600, width = 800)
